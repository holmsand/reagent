...
(deftest test-coll-class
  (is (= (rstr [:p {:class ['foo]}])
         (rstr [:p.foo])))
  (is (= (rstr [:p {:class #{'foo}} 1])
         (rstr [:p.foo 1])))
  (is (= (rstr [:p {:class ['foo 'foobar]} 2])
         (rstr [:p.foo.foobar 2])))
  (is (= (rstr [:p {:class ['foo 'foobar]} 3])
         (rstr [:p.foo.foobar 3])))
  (is (= (rstr [:p.bar {:class ['foo 'foobar]} 4])
         (rstr [:p.bar.foo.foobar 4])))
  (is (= (rstr [:p.bar {:class nil} 4])
         (rstr [:p.bar 4])))
  (is (= (rstr [:p.bar {:class []} 5])
         (rstr [:p.bar 5])))
  (is (= (rstr [:p {:class #{:foobar}} 6])
         (rstr [:p.foobar 6]))))

(deftest test-style-class
  (is (= (rstr [:p {:style {:class "foo"}}])
         (rstr [:p.foo])))
  (is (= (rstr [:p {:style {:class :foo}}])
         (rstr [:p.foo])))
  (is (= (rstr [:p {:style {:class 'foo}}])
         (rstr [:p.foo])))
  (is (= (rstr [:p {:style {:class "foo bar"}}])
         (rstr [:p.foo.bar])))
  (is (= (rstr [:p {:style {:class "foo bar"}}])
         (rstr [:p.foo.bar])))
  (is (= (rstr [:p {:style {:class nil}} "bar"])
         (rstr [:p "bar"])))
  (is (= (rstr [:p {:style {:class ['foo]}}])
         (rstr [:p.foo])))
  (is (= (rstr [:p {:style {:class #{'foo}}} 1])
         (rstr [:p.foo 1])))
  (is (= (rstr [:p {:style {:class ['foo 'foobar]}} 2])
         (rstr [:p.foo.foobar 2])))
  (is (= (rstr [:p {:style {:class ['foo 'foobar]}} 3])
         (rstr [:p.foo.foobar 3])))
  (is (= (rstr [:p.bar {:style {:class ['foo 'foobar]}} 4])
         (rstr [:p.bar.foo.foobar 4])))
  (is (= (rstr [:p.bar {:style {:class nil}} 4])
         (rstr [:p.bar 4])))
  (is (= (rstr [:p.bar {:style {:class []}} 5])
         (rstr [:p.bar 5])))
  (is (= (rstr [:p {:style {:class #{:foobar}}} 6])
         (rstr [:p.foobar 6])))
  (is (= (rstr [:p {:style {:class "foo"
                            :background-color 'black}}])
         (rstr [:p.foo {:style {"backgroundColor" "black"}}]))))
